...
async def get_current_user(request: Request, db: Annotated[AsyncSession, Depends(get_db)]) -> User:
    ...
    try:
        payload = decode(session_id, SECRET_KEY, algorithms=[ALGORITHM])#Декодирует токен, извлекает username и проверяет пользователя в Redis (для скорости) или в базе данных.
        ...
        result = await db.execute(select(User).where(User.username == username))#Возвращает объект User или выбрасывает ошибку 401.
        user = result.scalar_one_or_none()
        if user:
            await redis_client.setex(redis_key, 1800, str(user.id))  #Кэширует сессии на 30 минут
        if not user:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
